...
import { EnvelopeIcon, LockClosedIcon, UserCircleIcon, EyeIcon, EyeSlashIcon } from '@heroicons/react/24/outline';
    ...
    <div className="w-full max-w-md mx-auto px-4">
      <div className="bg-white/80 backdrop-blur-xl rounded-3xl p-8 shadow-2xl border border-gray-100/50">
        <div className="text-center mb-10">
          ...
          <p className="text-gray-500 text-sm font-medium">Please sign in to your accounts</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center justify-between px-1">
            <label className="flex items-center cursor-pointer group">
              <input
                type="checkbox"
                className="w-4 h-4 rounded border-gray-300 text-indigo-600 focus:outline-none cursor-pointer transition-colors"
              />
              <span className="ml-2 text-sm text-gray-500 group-hover:text-indigo-600 transition-colors">Remember me</span>
            </label>
            <Link to="/forgot-password" size="sm" className="text-sm text-indigo-600 hover:text-indigo-700 font-semibold transition duration-200">
              Forgot password?
            </Link>
          </div>
          ...
          <div className="mt-8">
            <button
              type="submit"
              disabled={loading}
              className="relative w-full py-4 px-4 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-bold rounded-xl 
          ...
          <div className="pt-6 border-t border-gray-100">
            <p className="text-center text-gray-500 text-sm font-medium">
              Are you an admin?{' '}
              <Link to="/admin/login" className="font-bold text-indigo-600 hover:text-indigo-700 transition duration-200">
                Admin Portal
              </Link>
            </p>
          </div>

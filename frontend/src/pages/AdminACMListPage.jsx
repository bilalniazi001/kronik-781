...
const AdminACMListPage = () => {
    ...
    const fetchUsers = useCallback(async () => {
        try {
            setLoading(true);
            const response = await adminService.getACMs(
            ...
            showError('Failed to fetch ACM accounts');
        } finally {
            setLoading(false);
        }
    ...
    const handleEdit = (user) => {
        navigate(`/admin/edit-acm/${user.id}`);
    };

    const handleDelete = async (userId) => {
        if (!window.confirm('Are you sure you want to delete this ACM account?')) return;

        try {
            await adminService.deleteUser(userId);
            showSuccess('ACM account deactivated successfully');
            fetchUsers();
        } catch (error) {
            showError('Failed to delete ACM');
        }
    };
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900">ACM Management</h1>
                    <p className="text-gray-600 mt-1">Assistant Center Managers</p>
                </div>
                <button
                    onClick={() => navigate('/admin/add-acm')}
                    className="btn-primary-premium group flex items-center"
                >
                    <div className="btn-shimmer"></div>
                    <PlusIcon className="w-5 h-5 mr-2 relative z-10" />
                    <span className="relative z-10">Add New ACM</span>
                </button>
            </div>
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center space-x-4">
                            ...
                            <div>
                                ...
                                <span className="inline-block px-2 py-0.5 mt-1 text-xs font-medium bg-teal-100 text-teal-800 rounded-full">
                                    ACM Role
                                </span>
                            </div>
                        </div>
...
export default AdminACMListPage;

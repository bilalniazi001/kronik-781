...
const AdminAddATL = () => {
    ...
    const fetchMetadata = async () => {
        try {
            const [usersRes, typesRes] = await Promise.all([
                adminService.getManagers(), // Team Leads
                hrService.getLeaveTypes()
            ]);
            setTls(usersRes.users || []);
            ...
            showError('Failed to load Team Leads or leave types');
        }
    };
    ...
    const fetchUserData = async () => {
        try {
            ...
            setFormData({
                ...
                phone: u.phone || '',
                cnic: u.cnic || '',
                designation: u.designation || 'Assistant Team Lead',
                department: u.department || 'Operations',
                role_type: u.role_type || 'atl',
                manager_id: u.manager_id || '',
                reporting_to: u.reporting_to || '',
                shift_type: u.shift_type || 'morning'
            });
        } catch (error) {
            showError('Failed to load ATL data');
        }
    };
    ...
    const handleManagerChange = (e) => {
        ...
        const m = tls.find(x => x.id === parseInt(mid));
        setFormData({
            ...formData,
            manager_id: mid,
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isEdit) {
                ...
                showSuccess(`ATL updated successfully!`);
            } else {
                await hrService.createEmployee(payload);
                showSuccess(`ATL account created successfully!`);
            }
            navigate('/admin/atls');
        } catch (error) {
            showError(error.message || `Failed to ${isEdit ? 'update' : 'create'} ATL account`);
        } finally {
            setLoading(false);
        }
    };
                ...
                <div className="bg-white/80 backdrop-blur-xl border border-gray-100 rounded-3xl p-8 shadow-xl">
                    <div className="mb-8 text-center">
                        <h2 className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-indigo-600 mb-2">
                             {isEdit ? 'Edit' : 'Add New'} ATL
                        </h2>
                        <p className="text-gray-500 text-sm font-medium">
                            {isEdit ? 'Update Assistant Team Lead information' : 'Create an ATL account with team responsibility'}
                        </p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="relative group">
                           ...
                           rounded-xl text-gray-700 focus:outline-none focus:border-indigo-600 transition-all font-medium"
                            >
                                <option value="">Select Team Lead (Reports To)</option>
                                {tls.map(m => (
                                    <option key={m.id} value={m.id}>{m.name}</option>
                                ))}
                            </select>
                            <p className="text-[10px] text-gray-400 mt-1 ml-1 font-medium italic">
                                * ATLs should report to Team Lead (TL).
                            </p>
                        </div>
                        ...
                        >
                            ...
                            <span className="relative z-10">
                                {loading ? (isEdit ? 'Updating...' : 'Creating...') : (isEdit ? 'Update ATL Account' : 'Create ATL Account')}
                            </span>
                        </button>
                    </form>
                </div>

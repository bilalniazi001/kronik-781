...
const AdminAddGM = () => {
    ...
    const fetchMetadata = async () => {
        try {
            const [usersRes, typesRes] = await Promise.all([
                adminService.getCEOs(),
                hrService.getLeaveTypes()
            ]);
            setCeos(usersRes.users || []);
            ...
            showError('Failed to load CEOs or leave types');
        }
    };
    ...
    const fetchUserData = async () => {
        try {
            ...
            setFormData({
                ...
                phone: u.phone || '',
                cnic: u.cnic || '',
                designation: u.designation || 'General Manager',
                department: u.department || 'Operations',
                role_type: u.role_type || 'gm',
                manager_id: u.manager_id || '',
                reporting_to: u.reporting_to || '',
                shift_type: u.shift_type || 'morning'
            });
        } catch (error) {
            showError('Failed to load GM data');
        }
    };
    ...
    const handleManagerChange = (e) => {
        ...
        const m = ceos.find(x => x.id === parseInt(mid));
        setFormData({
            ...formData,
            manager_id: mid,
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isEdit) {
                ...
                showSuccess(`GM account updated successfully!`);
            } else {
                await hrService.createEmployee(payload);
                showSuccess(`GM account created successfully!`);
            }
            navigate('/admin/gms');
        } catch (error) {
            showError(error.message || `Failed to ${isEdit ? 'update' : 'create'} GM account`);
        } finally {
            setLoading(false);
        }
    };
                ...
                <div className="bg-white/80 backdrop-blur-xl border border-gray-100 rounded-3xl p-8 shadow-xl">
                    <div className="mb-8 text-center">
                        <h2 className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-indigo-600 mb-2">
                             {isEdit ? 'Edit' : 'Add New'} GM
                        </h2>
                        <p className="text-gray-500 text-sm font-medium">
                            {isEdit ? 'Update General Manager information' : 'Create a GM account with high-level oversight'}
                        </p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        >
                            ...
                            <span className="relative z-10">
                                {loading ? (isEdit ? 'Updating...' : 'Creating...') : (isEdit ? 'Update GM Account' : 'Create GM Account')}
                            </span>
                        </button>
                    </form>
                </div>

...
const AdminAddHR = () => {
    ...
    const fetchUserData = async () => {
        try {
            ...
            setFormData({
                ...
                designation: u.designation || 'HR Manager',
                shift_type: u.shift_type || 'morning'
            });
        } catch (error) {
            showError('Failed to load HR details');
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const payload = { ...formData, leave_quota: leaveQuota, role_type: 'hr' };
            if (isEdit) {
                await adminService.updateUser(id, payload);
                showSuccess(`HR account updated successfully!`);
            } else {
                await hrService.createEmployee(payload); // Using createEmployee as it handles leave_quota
                showSuccess(`HR account created successfully!`);
            }
            navigate('/admin/hr');
        } catch (error) {
            showError(error.message || `Failed to ${isEdit ? 'update' : 'create'} HR account`);
        } finally {
            setLoading(false);
        }
    };
                ...
                <div className="bg-white/80 backdrop-blur-xl border border-gray-100 rounded-3xl p-8 shadow-xl">
                    <div className="mb-8 text-center">
                        <h2 className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-indigo-600 mb-2">
                            {isEdit ? 'Edit' : 'Add New'} HR Personnel
                        </h2>
                        <p className="text-gray-500 text-sm font-medium">
                            {isEdit ? 'Update HR account details' : 'Create an HR account to manage employees and leave policies'}
                        </p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="md:col-span-2">
                                <FormInput
                                    ...
                                    icon={BriefcaseIcon}
                                    name="designation"
                                    placeholder="e.g. HR Manager, HR Officer"
                        ...
                        >
                            ...
                            <span className="relative z-10">
                                {loading ? (isEdit ? 'Updating...' : 'Creating...') : (isEdit ? 'Update HR Account' : 'Create HR Account')}
                            </span>
                        </button>
                    </form>
                </div>

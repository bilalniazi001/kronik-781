...
const AdminAddTeamLead = () => {
    ...
    const fetchUserData = async () => {
        try {
            ...
            // Note: If backend provides leave balances, we might want to pre-fill leaveQuota too
            // But usually balances are in a separate table. For now focus on basic fields.
        } catch (error) {
    ...
    const handleManagerChange = (e) => {
        ...
        const m = managers.find(x => x.id === parseInt(mid));
        setFormData({
            ...formData,
            manager_id: mid,
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isEdit) {
                ...
                showSuccess(`Team Lead account updated successfully!`);
            } else {
                await hrService.createEmployee(payload);
                showSuccess(`Team Lead account created successfully!`);
            }
            navigate('/admin/team-leads');
        } catch (error) {
            showError(error.message || `Failed to ${isEdit ? 'update' : 'create'} Team Lead account`);
        } finally {
            setLoading(false);
        }
    };
                ...
                <div className="bg-white/80 backdrop-blur-xl border border-gray-100 rounded-3xl p-8 shadow-xl">
                    <div className="mb-8 text-center">
                        <h2 className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-indigo-600 mb-2">
                            {isEdit ? 'Edit' : 'Add New'} Team Lead (TL)
                        </h2>
                        <p className="text-gray-500 text-sm font-medium">
                            {isEdit ? 'Update Team Lead information' : 'Create a Team Lead account with team oversight'}
                        </p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="relative group">
                               ...
                               rounded-xl text-gray-700 focus:outline-none focus:border-indigo-600 transition-all font-medium"
                                >
                                    <option value="manager">Team Lead (TL)</option>
                                </select>
                            </div>
                        ...
                        <div className="relative group">
                           ...
                           rounded-xl text-gray-700 focus:outline-none focus:border-indigo-600 transition-all font-medium"
                            >
                                <option value="">Select Approver (Reports To)</option>
                                {managers.map(m => (
                            ...
                            <p className="text-[10px] text-gray-400 mt-1 ml-1 font-medium italic">
                                * Team Leads (TLs) should report to ACM or GM.
                            </p>
                        </div>
...
export default AdminAddTeamLead;

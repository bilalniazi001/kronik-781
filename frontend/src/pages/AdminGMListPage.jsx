...
const AdminGMListPage = () => {
    ...
    const fetchUsers = useCallback(async () => {
        try {
            setLoading(true);
            const response = await adminService.getGMs(
            ...
            showError('Failed to fetch General Manager accounts');
        } finally {
            setLoading(false);
        }
    ...
    const handleEdit = (user) => {
        navigate(`/admin/edit-gm/${user.id}`);
    };

    const handleDelete = async (userId) => {
        if (!window.confirm('Are you sure you want to delete this GM account?')) return;

        try {
            await adminService.deleteUser(userId);
            showSuccess('GM account deactivated successfully');
            fetchUsers();
        } catch (error) {
            showError('Failed to delete GM');
        }
    };
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900">General Manager (GM) Management</h1>
                    <p className="text-gray-600 mt-1">View and manage organization GMs</p>
                </div>
                <button
                    onClick={() => navigate('/admin/add-gm')}
                    className="btn-primary-premium group flex items-center"
                >
                    <div className="btn-shimmer"></div>
                    <PlusIcon className="w-5 h-5 mr-2 relative z-10" />
                    <span className="relative z-10">Add New GM</span>
                </button>
            </div>
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center space-x-4">
                            ...
                            <div>
                                ...
                                <span className="inline-block px-2 py-0.5 mt-1 text-xs font-medium bg-orange-100 text-orange-800 rounded-full">
                                    General Manager
                                </span>
                            </div>
                        </div>
...
export default AdminGMListPage;
